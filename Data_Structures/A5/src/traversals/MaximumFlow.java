...
public class MaximumFlow<V> {
    ...
    public int runMaxFlow() {
        Optional<EdgePath<V>> shortest=residual.shortestWeightedPath(source, destination);
        if (shortest.isEmpty())
            return maxFlow;
        Weight shortestWeight=Weight.of(Integer.MAX_VALUE);
        for (Edge<V> edgeWeight : shortest.get()){
            if (edgeWeight.weight().compareTo(shortestWeight)<0)
                shortestWeight=edgeWeight.weight();
        }
        maxFlow+=shortestWeight.asInt();
        for (Edge<V> edge : shortest.get()){
            residual.subtractEdgeWeight(edge, shortestWeight);
            residual.addEdgeWeight(edge.flip(), shortestWeight);
        }
        return runMaxFlow();
    }
}