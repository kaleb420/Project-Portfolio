...
//                 - For each neighbor in-bounds & not visited: mark visited, append letter, enqueue
        Queue<Position> position=new LinkedList<>();
        boolean[][] visited= new boolean[rows][cols];
        position.add(new Position(startRow, startCol, grid[startRow][startCol] + ""));
        /*while (!position.isEmpty()){
            position.remove(grid[startRow][startCol] + "");
            int[][] possibleDirections={{1,0}, {0,1}, {-1,0}, {0,-1}};
            if (position.size()>=2)
                foundWords.add(position.toString());
            for (int[] index : possibleDirections){
                int newRow=startRow+index[0];
                int newCol=startCol+index[1];
                if (isInBounds(newRow, newCol) && !visited[newRow][newCol]){
                    visited[newRow][newCol]=true;
                    position.add(new Position(newRow, newCol, grid[newRow][newCol] + ""));
                }
            }
        } */
    }
