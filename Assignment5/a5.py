...
def ctr(n,acc1=9,acc2=0):
    if n==0:
        ...
        return acc1,9*c(n-1)+10**(n-1,acc2)-c(n-1)
...
def value(portfolio, market):
    p_value=(portfolio['stock']['x'][0]*portfolio['stock']['x'][1])
    m_value=market['x']*portfolio['stock']['x'][1]
...
def break_code(secret_code, combinations):
    if secret_code==combinations:
        return secret_code
    else:
        break_code(secret_code,combinations)
