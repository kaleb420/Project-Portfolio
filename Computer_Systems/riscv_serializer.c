...
int generate_load_instruction(int width, int destination, int address, int offset) {
	width=(width & 0x7) << 12;
	destination=(destination & 0x1f) << 7;
	address=(address & 0x1f) << 15;
	offset=(offset & 0xfff) << 20;
	int opcode=0x0 << 2;
	return width | destination | address | offset | opcode | 0x3;
}

int generate_store_instruction(int width, int address, int source, int offset) {
	width=(width & 0x7) << 12;
	address=(address & 0x1f) << 15;
	source=(source & 0x1f) << 20;
	int mask_one=0x1f;
	int mask_two=0xfe0;
	int offset_one=(offset & mask_one) << 7;
	int offset_two=(offset & mask_two) << 20;
	int opcode=0x8 << 2;
	return width | address | source | offset_one | offset_two | opcode | 0x3; 
}

int generate_math_instruction(int function, int a, int b, int destination) {
	function=(function & 0x7) << 12;
	a=(a & 0x1f) << 15;
	b=(b & 0x1f) << 20;
	destination=(destination & 0x1f) << 7;
	int opcode=0xC << 2;
	return function | a | b | destination | opcode | 0x3;
}

int generate_constant_instruction(int value, int destination) { 
	value=(value & 0xfffff) << 12;
	destination=(destination & 0x1f) << 7;
	int opcode=0xD << 2;
	return value | destination | opcode | 0x3;
}
