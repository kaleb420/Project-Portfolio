...
void kreverse(char *str, char *output){
	int length=(int) klen(str);
	...
	for (u32 i=0; *(format+i)!='\0'; i++){
		if (*(format+i)=='%') {
			if (*(format+i+1)=='d') {
				...
				temp[j]='\0';
				if (j==0)
					uart->write(uart, 0, "0", 2);
				else {
					char output[klen(temp)+1];
					kreverse(temp, output);
					uart->write(uart, 0, output, (u64) klen(output));
				}
			}
				...
				uart->write(uart, 0, location, 1);
			}
			else if (*(format+i+1)=='x') {
				int va_int=va_arg(ap, int);
				uart->write(uart, 0, "0x", 2);
