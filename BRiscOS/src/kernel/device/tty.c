...
static i64 tty_gets(device_t* dev, [[gnu::unused]] u64 handle, char* buffer, u64 len) {
	...
	u64 i=0;
	lock_mutex(&tty->lock);
	while (tty->in.count==0){
                enqueue_futex(&tty->lock, tty->in.avail);
		release_mutex(&tty->lock);
		resched();
		lock_mutex(&tty->lock);
	}
        u32 head=tty->in.head;
	while (i<tty->in.count && i<len){
		buffer[i]=tty->in.buffer[(head+i)%TTY_BUFFLEN];
		i++;
		if (buffer[i-1]=='\n')
			break;
	}
	tty->in.count-=(u32) i;
	tty->in.avail-= (u32) i;
	tty->in.head=(tty->in.head+i)%TTY_BUFFLEN;
	buffer[i]='\0';
	release_mutex(&tty->lock);
	if (i==0)
		return -1;
...
static i64 tty_puts(device_t* dev, [[gnu::unused]] u64 handle, const char* buffer, u64 lens) {
	tty_t* tty = (tty_t*)dev->context;
	device_t* uart = dev->origin;
	u64 i=0;
	lock_mutex(&tty->lock);
	while (tty->out.count==TTY_BUFFLEN){
		enqueue_futex(&tty->lock, tty->out.avail);
		release_mutex(&tty->lock);
		resched();
		lock_mutex(&tty->lock);
	}
	u32 tail=tty->out.count+tty->out.head;
	while (i<lens && buffer[i]!='\0'){
		tty->out.buffer[(tail+i)%TTY_BUFFLEN]=buffer[i];
		i++;
	}
	tty->out.avail-= (u32) i;
	tty->out.count+= (u32) i;
	release_mutex(&tty->lock);
	set_uart_transmit(uart, true);
	if (i==0)
		return -1;
	return (i64) i; 
}
...
static void handle_tty(device_t* dev) {
	...
	if (status == UART_RX_INTR) {
		...
		if (ch=='\r')
			ch='\n';
		tty->in.buffer[tty->in.count+tty->in.head]=ch;
		tty->in.count++;
		if (tty->flags==TTY_ECHO){
			tty->out.buffer[tty->out.count+tty->out.head]=ch;
			tty->out.count++;
			tty->out.avail--;
			set_uart_transmit(uart, true);
		}
		if (ch=='\n'){
			tty->in.avail++;
			dequeue_futex(&tty->lock, tty->out.avail);
		}
		release_mutex(&tty->lock);
  	}
	/*  Interrupt indicates the UART is ready to output a character */
	else if (status == UART_TX_INTR) {
		char ch=tty->out.buffer[tty->out.head];
		uart->addr[UART_RW]=ch;
		lock_mutex(&tty->lock);
		tty->out.count--;
		tty->out.avail++;
                tty->out.head=(tty->out.head+1)%TTY_BUFFLEN;
		if (tty->out.count==0)
			set_uart_transmit(uart, false);
		dequeue_futex(&tty->lock, tty->out.avail);
		release_mutex(&tty->lock);
  	}
}
