...
syscall_t join(u32 threadid) {
	if (NUM_THREADS<=threadid || thread_table[threadid].state==TH_FREE || threadid==get_current_thread()) 
		return -1; 
	while (thread_table[threadid].lock>0){ 
		thread_table[threadid].lock--; 
	}
	while (thread_table[threadid].state!=TH_DEFUNCT){ 
		...
		thread_table[get_current_thread()].state=TH_SUSPEND;
		resched(); 
	} 
	resched(); 
...
syscall_t resume(u32 threadid) {
	if (NUM_THREADS<=threadid || thread_table[threadid].state!=TH_SUSPEND)
		return -1;
	thread_table[threadid].state=TH_READY;
	list_insert_thread(&ready_list, threadid, thread_table[threadid].priority);
	resched();
	return threadid;
}
