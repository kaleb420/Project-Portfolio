class Contact:

    def __init__(self, nameList, phoneNumber, email, birthday):
        """
        Variables Sent:
            Name, Phone number, Email, Birthdate

        Create Instance variables:
            - First Name:   first
            - Middle Name:  middle
            - Last Name:    last
            - Phone Number: mobile
            - Email:        email
            - BirthDate:    birthdate
        """
        if len(nameList)>2:
            self.first=nameList[0]
            self.middle=nameList[1]
            self.last=nameList[2]
        else:
            self.first=nameList[0]
            self.middle=None
            self.last=nameList[1]
        self.mobile=phoneNumber
        self.email=email 
        self.BirthDate=birthday

    def getName(self):
        """
        Return the name as a string like this "first middle last"
        The middle name might not exist
        """
        if (self.middle!=None):
            return self.first + '' + self.middle + '' +self.last
        else:
            return self.first + '' + self.last

    def __str__(self):
        """
        Returns a string representing the contact:
        > Contact: Josh Baker - 3178675309
        """
        return ' > Ctonact: {} {} - {}'.format(self.first,self.last,self.mobile)
    
    def __repr__(self):
        """
        Provided: 
        - Feel free to read more about it here: https://stackoverflow.com/questions/1436703/difference-between-str-and-repr
        """
        return self.__str__() # This is provided. Helps with something printing a list of contact items. 

    def call(self):
        """
        Print "Dialing ..." and then when they dont pick up
        the user should be prompted to input a message and then it should
        be printed
        """
        print("Dialing...{}".format(self.first))

    def sendBirthdayText(self):
        """
        There should be a premade birthday text that uses the name 
        and then prints it out when this is called
        """
        print('Happy birthday,  {}'.format(self.first))


    def sendText(self, message):
        """
        Given a message a text to the person
        """
        print('To {}: '.format(self.first))
        print('\t{}'.format(message))

    def updateNumber(self, phoneNumber):
        """
        Take in a new phone number and replace the user's current phone number
        """
        self.mobile=phoneNumber



    
shorter_name=['john','smith']
name_data=['kaleb','jon','rob']
phone_data=2412509
email_data='foaei@gmail.com'
birthday_data='4/20/04'

kaleb_object=Contact(name_data,phone_data,email_data,birthday_data)
john_object=Contact(shorter_name,phone_data,email_data,birthday_data)

print(john_object.getName())

print(str(john_object))
print('repr function gives: ' + repr(john_object))

print('before ' + str(kaleb_object.mobile))
kaleb_object.updateNumber(111111111111111111111)
print('after ' + str(kaleb_object.mobile))

kaleb_object.sendBirthdayText()

kaleb_object.sendText('hello how are you')

kaleb_object.call()